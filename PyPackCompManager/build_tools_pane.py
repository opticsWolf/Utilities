...
class BuildToolsMixin:
    ...
    def _build_tools_dock(self):
        ...
        self.maturin_target_combo.setToolTip(
            "The system architecture to compile for (e.g., x86_64, aarch64)."
        )
        ...
        self.maturin_target_combo.currentTextChanged.connect(
        ...
        self.rust_cpu_combo.setToolTip(
            "Specific CPU architecture to optimize for (-C target-cpu). 'native' uses the host's CPU."
        )
        ...
        self.rust_features_edit.setToolTip(
            "Hardware specific target features to enable (-C target-feature)."
        )
        ...
        self.maturin_auditwheel_combo.setToolTip(
            "Linux only: Repair the wheel for broader compatibility using auditwheel."
        )
        ...
        self.maturin_release_cb.setToolTip(
            "Build the Python wheel in release mode with optimizations."
        )
        self.maturin_strip_cb = QCheckBox("--strip")
        self.maturin_strip_cb.setToolTip(
            "Strip symbols from the compiled library for minimum file size."
        )
        self.maturin_sdist_cb = QCheckBox("--sdist")
        self.maturin_sdist_cb.setToolTip(
            "Build a source distribution (.tar.gz) alongside the wheel."
        )
        self.maturin_no_default_features_cb = QCheckBox("--no-default-features")
        self.maturin_no_default_features_cb.setToolTip(
            "Disable the default Rust features defined in the project's Cargo.toml."
        )

        maturin_extra_layout = QHBoxLayout()
        maturin_extra_layout.addWidget(QLabel("Extra args:"))
        self.maturin_extra_edit = QLineEdit()
        self.maturin_extra_edit.setToolTip(
            "Any additional arguments to pass to 'maturin build'."
        )
        ...
        self._setup_button(
            self.run_maturin_btn,
            "Build Python wheels using Maturin in the selected environment.",
        )
        self._style_critical_button(self.run_maturin_btn, "#6A5ACD", "#5F4FBD")
        self.run_maturin_btn.clicked.connect(self.run_maturin)
        
        # Add the group to the tab layout
        maturin_layout.addWidget(maturin_group)
        # Add the button to the tab layout, OUTSIDE the group
        maturin_layout.addWidget(self.run_maturin_btn)
        # Add the stretch at the very bottom
        ...
        self.cargo_release_cb.setToolTip(
            "Build artifacts in release mode, with optimizations (slower build, faster execution)."
        )
        self.cargo_verbose_cb = QCheckBox("--verbose")
        self.cargo_verbose_cb.setToolTip(
            "Use verbose output, providing more detailed logging during the build."
        )
        self.cargo_all_features_cb = QCheckBox("--all-features")
        self.cargo_all_features_cb.setToolTip(
            "Activate all available features of the selected package defined in Cargo.toml."
        )
        self.cargo_no_default_features_cb = QCheckBox("--no-default-features")
        self.cargo_no_default_features_cb.setToolTip(
        ...
        self.cargo_extra_edit.setToolTip(
            "Any additional arguments you want to pass to the Cargo command."
        )
        ...
        self._setup_button(
            self.run_cargo_btn,
            "Execute the configured Cargo command in the selected environment.",
        )
        ...
        # Add the button to the tab layout, OUTSIDE the group
        cargo_layout.addWidget(self.run_cargo_btn)
        # Add the stretch at the very bottom
        ...
        # Note: the main scroll area won't collapse the tabs due to the stretches inside them
    ...
    def _load_build_settings(self):
        ...
        if idx >= 0:
            self.cargo_cmd_combo.setCurrentIndex(idx)
        ...
        self.cargo_all_features_cb.setChecked(
            self.settings.get("cargo_all_features", False)
        )
        self.cargo_no_default_features_cb.setChecked(
            self.settings.get("cargo_no_default_features", False)
        )
        ...
        self.maturin_no_default_features_cb.setChecked(
            self.settings.get("maturin_no_default_features", False)
        )

        aw_cmd = self.settings.get("maturin_auditwheel", "(default)")
        idx = self.maturin_auditwheel_combo.findText(aw_cmd)
        if idx >= 0:
            self.maturin_auditwheel_combo.setCurrentIndex(idx)
    ...
    def _save_build_settings(self):
        ...
        self.settings.set(
            "rust_target_features", self.rust_features_edit.text().strip()
        )
        ...
        self.settings.set(
            "cargo_no_default_features", self.cargo_no_default_features_cb.isChecked()
        )
        ...
        self.settings.set(
            ...
            self.maturin_no_default_features_cb.isChecked(),
        )
        self.settings.set(
            "maturin_auditwheel", self.maturin_auditwheel_combo.currentText()
        )
