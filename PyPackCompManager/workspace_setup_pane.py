...
class WorkspaceMixin:
    ...
    def _build_workspace_dock(self):
        ...
        self._setup_button(
            browse_env_base_btn, "Browse for your environment base directory."
        )
        ...
        self._setup_button(
            refresh_env_btn, "Refresh the list of available environments."
        )
    ...
    def get_selected_env_data(self):
        ...
        if not env_data:
            QMessageBox.warning(
                self, "No Environment", "Please select a valid environment."
            )
            return None
        return env_data
    ...
    def browse_env_base(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Select Environment Base Folder", start_path
        )
        if folder:
    ...
    def browse_folder(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Select Project Folder", start_path
        )
        if folder:
    ...
    def _get_conda_executable(self, base_dir):
        parent_dir = (
            ...
            if os.path.basename(base_dir) == "envs"
            else base_dir
        )
        candidates = []
        if os.name == "nt":
            ...
            if is_venv(base_dir) or is_conda_env(base_dir):
                ...
                try:
                    for entry in os.scandir(base_dir):
                        if entry.is_dir() and (
                            is_venv(entry.path) or is_conda_env(entry.path)
                        ):
                            venvs.append(
